...
describe('recipe scraper utils', () => {
  ...
  it('should invalidate prepTime if it is an array', () => {
    const validation = () => validate(getValidRecipePageInfo({ prepTime: ['5 mins'] }));
    expect(validation).toThrow(ScrapeError);
  });
  ...
  it('should invalidate servings if it is an array', () => {
    const validation = () => validate(getValidRecipePageInfo({ servings: ['4'] }));
    expect(validation).toThrow(ScrapeError);
  });

  it('should invalidate servings if it is an invalid number', () => {
    const validation = () => validate(getValidRecipePageInfo({ servings: '4g' }));
    expect(validation).toThrow(ScrapeError);
  });
  ...
  it('should invalidate name if it is an array', () => {
    const validation = () => validate(getValidRecipePageInfo({ name: ['5 mins'] }));
    expect(validation).toThrow(ScrapeError);
  });

  it('should invalidate ingredients if it is an empty array', () => {
    const validation = () => validate(getValidRecipePageInfo({ ingredients: [] }));
    expect(validation).toThrow(ScrapeError);
  });

  it('should invalidate ingredients if it is not an array', () => {
    const validation = () => validate(getValidRecipePageInfo({ ingredients: '5 mins' }));
    expect(validation).toThrow(ScrapeError);
  });
