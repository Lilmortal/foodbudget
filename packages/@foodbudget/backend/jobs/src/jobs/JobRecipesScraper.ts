...
export class JobRecipesScraper implements Job {
  ...
  constructor({ serviceManager, recipeScrapers }: JobScraperParams) {
  ...
  async scrape(
    scrapedElements: ScrapedRecipeHTMLElements[],
  ): Promise<Omit<Recipe, 'id'>[]> {
    const result = await Promise.all(
      this.recipeScrapers.map(async (scraper) =>
        scraper.scrape(scrapedElements),
      ),
    );
  ...
  async save(
    recipes: Omit<Recipe, 'id'> | Omit<Recipe, 'id'>[],
  ): Promise<void> {
  ...
  // async notify(
  //   recipes: Omit<Recipe, 'id'> | Omit<Recipe, 'id'>[],
  //   mailParticipants: Pick<Mail, 'from' | 'to'>,
  // ): Promise<void> {
  ...
  async start(config: Config): Promise<void> {
    ...
    await Promise.all(
      scrapedRecipes.map(async (scrapedRecipe) => {
        logger.info('Successfully scraped recipe.');
        logger.info(
          `Scraped recipe: ${JSON.stringify(scrapedRecipe, null, 2)}`,
        );
        ...
        // await this.notify(scrapedRecipe, {
