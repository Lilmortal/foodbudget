...
const isSocialLoginData = (user: Express.User): user is SocialLoginData =>
  !!(user as SocialLoginData).email &&
  !!(user as SocialLoginData).id &&
  !!(user as SocialLoginData).strategy;
...
export const handleTokenVerification = (
  ...
  if (req.user && isSocialLoginData(req.user)) {
    await authServices.login(
      getSocialLoginRequest(req.user.strategy, req.user.id, req.user.email),
    );
