...
describe('auth queries', () => {
  ...
  it('should throw an error if refresh token does not exist', async () => {
    ...
    expect(res.errors).toEqual([
      new GraphQLError('refresh token either expired or does not exist.'),
    ]);
  });
  ...
  it('should renew refresh token and return a refreshed access token', async () => {
    ...
    const refreshToken = sign(
      { userId: '4', expireTimeInUtc: expireTimeInMs },
      mockRefreshSecret,
    );
    ...
    expect(res.data).toEqual({
      renewToken:
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI0Iiwic2NvcGUiOltdLCJleHBpcmVUaW1lSW5VdGM' +
    ...
    expect(mockCookieRes.mock.results[0].value).toEqual({
      name: 'refresh-token',
      option: {
        expires: new Date('2020-11-04T04:39:27.905Z'),
        httpOnly: true,
